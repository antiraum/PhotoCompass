...
public final class PhotosModel {
    ...
    private static final PhotosModel _instance = new PhotosModel();
    ...
    private final RemoteCallbackList<IPhotosModelCallback> _remoteCallbacks = new RemoteCallbackList<IPhotosModelCallback>();
     ...
     */
    public static PhotosModel getInstance() {
     ...
     */
    public void registerCallback(final IPhotosModelCallback cb) {
     ...
     * @param cb Callback object.
     */
    public void unregisterCallback(final IPhotosModelCallback cb) {
     ...
     * photos array with the images on the device that can be used in the application.
     * 
     * @param activity Calling {@link Activity}. Needed for calling
     *        {@link Activity#managedQuery(Uri, String[], String, String[], String)} on {@link MediaStore.Images}.
     */
...
//            Log.d(PhotoCompassApplication.LOG_TAG, "Photos: idCol = "+idCol+", latCol = "+latCol+", lngCol = "+lngCol+", dateCol = "+dateCol);
            ...
            for (int j = 0; j < numrows; j++) {
                ...
                thumbCursor = activity.managedQuery(thumbUris[i], thumbColumns, Thumbnails.IMAGE_ID + "='" + id + "'",
                                                    null, null);
...
//        Log.d(PhotoCompassApplication.LOG_TAG, "Photos: _mergePhotos: #photos = "+_photos.size()+", #dummies = "+_dummies.size());
        ...
        for (final SparseArray<Photo> photos : new SparseArray[] {_photos, _dummies}) {
            ...
            for (int i = 0; i < numPhotos; i++) {
                ...
                toMergeWith.clear();
                for (final SparseArray<Photo> others : new SparseArray[] {_photos, _dummies}) {
                    numOthers = others.size();
                    for (int j = 0; j < numOthers; j++) {
                        otherId = others.keyAt(j);
                        if (otherId == photoId || mergedPhotos.contains(otherId)) continue;
...
//    	Log.d(PhotoCompassApplication.LOG_TAG, "Photos: _isPhotoVisible: id = "+photo.getId());
        if (limitByDistance && photo.distance < appModel.minDistance || photo.distance > appModel.maxDistance) //	    	Log.d(PhotoCompassApplication.LOG_TAG, "Photos: _isPhotoVisible: photo is too close or too far away: photo.distance = "+photo.distance);
            return false;
        final long photoAge = photo.getAge();
        if (limitByAge && photoAge < appModel.minAge || photoAge > appModel.maxAge) //	    	Log.d(PhotoCompassApplication.LOG_TAG, "Photos: _isPhotoVisible: photo is too young or too old");
//	    	Log.d(PhotoCompassApplication.LOG_TAG, "Photos: _isPhotoVisible: photoAge = "+photoAge+", minAge = "+appModel.minAge+", maxAge = "+appModel.maxAge);
    ...
    public void updatePhotoProperties(final double lat, final double lng, final double alt) {
        ...
        for (final SparseArray<Photo> photos : new SparseArray[] {_photos, _dummies}) {
            numPhotos = photos.size();
            for (int i = 0; i < numPhotos; i++)
                photos.valueAt(i).updateDistanceDirectionAndAltitudeOffset(lat, lng, alt);
        }
...
//	    _dummies.append(R.drawable.fit_11067049, new Photo(R.drawable.fit_11067049, Location.convert("50:45:8.10"), Location.convert("7:12:28.59"), 105, dateTime));
        _dummies.append(R.drawable.fit_4138394, new Photo(R.drawable.fit_4138394, Location.convert("50:45:20.71"),
...
//	    _dummies.append(R.drawable.fit_11092935, new Photo(R.drawable.fit_11092935, Location.convert("50:45:23.27"), Location.convert("7:12:16.96"), 160, dateTime));
        _dummies.append(R.drawable.fit_12610213, new Photo(R.drawable.fit_12610213, Location.convert("50:45:19.29"),
...
//	    _dummies.append(R.drawable.fit_14308427, new Photo(R.drawable.fit_14308427, Location.convert("50:44:56.21"), Location.convert("7:13:16.02"), 120, dateTime));
        _dummies.append(R.drawable.fit_8503628, new Photo(R.drawable.fit_8503628, Location.convert("50:44:29.47"),
...
//	    _dummies.append(R.drawable.fit_3038737, new Photo(R.drawable.fit_3038737, Location.convert("50:43:49.21"), Location.convert("7:13:11.66"), 123, dateTime));
        _dummies.append(R.drawable.fit_4410168, new Photo(R.drawable.fit_4410168, Location.convert("50:45:22.80"),
...
//		_dummies.append(R.drawable.fit_12610204, new Photo(R.drawable.fit_12610204, Location.convert("50:45:17.73"), Location.convert("7:12:51.92"), 126, dateTime));
        _dummies.append(R.drawable.fit_14308344, new Photo(R.drawable.fit_14308344, Location.convert("50:44:57.02"),
                                                           Location.convert("7:13:24.18"), 127, dateTime));
        _dummies.append(R.drawable.fit_1798151678_af72c8f78d, new Photo(R.drawable.fit_1798151678_af72c8f78d,
                                                                        Location.convert("50:44:58"),
                                                                        Location.convert("7:12:21"), 126, dateTime));
        _dummies.append(R.drawable.fit_2580082727_1faf043ec1, new Photo(R.drawable.fit_2580082727_1faf043ec1,
                                                                        Location.convert("50:44:5"),
                                                                        Location.convert("7:12:19"), 125, dateTime));
        _dummies.append(R.drawable.fit_2417313476_d588a4e2b5, new Photo(R.drawable.fit_2417313476_d588a4e2b5,
