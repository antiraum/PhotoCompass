 ...
 * {@link de.fraunhofer.fit.photocompass.activities.FinderActivity} and displays
 * the UI controls.
 */
public final class ControlsView extends SimpleAbsoluteLayout {

	private static final int CONTROL_END_PADDING = 8; // padding at the top of
	// the distance control
	// and
	// at the right of the age control
	public static final int CONTROL_SIDE_PADDING = 5; // padding at the left of
	// the distance control
	// and
	// at the bottom of the age control
	private static final int BOTTOM_LEFT_PADDING = 39; // padding at the bottom
	// of the distance
	// control and
	// at the left of the age control
	public static final int DISTANCE_CONTROL_WIDTH = 120; // width of the
	// distance seek bar
	// (including text
	// labels)
	private static final int AGE_CONTROL_HEIGHT = 58; // height of the age seek
	// bar (including text
	// labels)
	public static final int BOTTOM_EXTRA_PADDING = 12; // extra bottom padding,
	// because the touch
	// screen on the G1
	// doesn't work well at the bottom

	DoubleSeekBar distanceSlider = null; // package scoped for faster access by
	// inner classes
	DoubleSeekBar ageSlider = null; // package scoped for faster access by inner

	// classes

	/**
	 * Constructor. Sets up the controls and registers as a callback at the
	 * application model.
	 * 
	 * @param context
	 * @param availableWidth
	 *            Maximum width for this view (the display width).
	 * @param availableHeight
	 *            Maximum height for this view (the display height).
	 */
	public ControlsView(final Context context, final int availableWidth,
			final int availableHeight, boolean showDistanceControl,
			boolean showAgeControl, boolean lightBackground) {
		super(context);

		// distance slider
		if (showDistanceControl) {
			distanceSlider = new VerticalDoubleSeekBar(context,
					new IDoubleSeekBarCallback() {
			...
			final int bottomPadding = showAgeControl ? BOTTOM_LEFT_PADDING
					: CONTROL_END_PADDING;
			distanceSlider.setLayoutParams(new LayoutParams(
					DISTANCE_CONTROL_WIDTH, availableHeight
							- CONTROL_END_PADDING - bottomPadding
							- BOTTOM_EXTRA_PADDING, CONTROL_SIDE_PADDING,
					CONTROL_END_PADDING));
		...
		// age slider
		if (showAgeControl) {
			ageSlider = new HorizontalDoubleSeekBar(context,
					new IDoubleSeekBarCallback() {
						final ApplicationModel model = ApplicationModel
								.getInstance();
			...
			final int xPos = showDistanceControl ? BOTTOM_LEFT_PADDING
					: CONTROL_END_PADDING;
			ageSlider.setLayoutParams(new LayoutParams(availableWidth
					- CONTROL_END_PADDING - xPos, AGE_CONTROL_HEIGHT, xPos,
					availableHeight - AGE_CONTROL_HEIGHT - CONTROL_SIDE_PADDING
