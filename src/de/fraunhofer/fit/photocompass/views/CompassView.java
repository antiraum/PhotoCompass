...
public final class CompassView extends SimpleAbsoluteLayout {
    ...
    private final CompassLineView _lineView;
    ...
    public CompassView(final Context context, final int availableWidth, final int availableHeight) {
        ...
        TextView tv;
        for (final int deg : _directionDegrees) {
...
//    	Log.d(PhotoCompassApplication.LOG_TAG, "CompassView: update: direction = "+direction);
        ...
        TextView dirLabel;
        for (final int deg : _directionDegrees) {
            _directionPositions.put(deg, Math.round(AVAILABLE_WIDTH / 2 + (deg - _direction) * DEGREE_WIDTH));
            ...
            if (dirLabel.getLayoutParams() != null && (dirLabel.getRight() < 0 && layoutParams.x + LABEL_WIDTH < 0) || // left of screen
