...
public class FinderActivity extends Activity {
	...
	private ServiceConnection _locationServiceConn = new ServiceConnection() {
		...
		public void onServiceConnected(ComponentName className, IBinder service) {
			Log.d(PhotoCompassApplication.LOG_TAG,
					"FinderActivity: connected to location service");
			...
			try {
				...
				Log
						.e(PhotoCompassApplication.LOG_TAG,
								"FinderActivity: failed to register to location service");
			}
		}

		public void onServiceDisconnected(ComponentName name) {
			Log.d(PhotoCompassApplication.LOG_TAG,
					"FinderActivity: disconnected from location service");
	...
	private ILocationServiceCallback _locationServiceCallback = new ILocationServiceCallback.Stub() {
		...
		public void onLocationEvent(double latitude, double longitude,
				double altitude) {
			// Log.d(PhotoCompassApplication.LOG_TAG,
			// "FinderActivity: received event from location service");
	...
	private ServiceConnection _orientationServiceConn = new ServiceConnection() {
		...
		public void onServiceConnected(ComponentName className, IBinder service) {
			Log.d(PhotoCompassApplication.LOG_TAG,
					"FinderActivity: connected to orientation service");
			...
			try {
				_orientationService
						.registerCallback(_orientationServiceCallback);
			} catch (DeadObjectException e) {
				// service crashed
			} catch (RemoteException e) {
				Log
						.e(PhotoCompassApplication.LOG_TAG,
								"FinderActivity: failed to register to orientation service");
			}
		}

		public void onServiceDisconnected(ComponentName name) {
			Log.d(PhotoCompassApplication.LOG_TAG,
					"FinderActivity: disconnected from orientation service");
	...
	private IOrientationServiceCallback _orientationServiceCallback = new IOrientationServiceCallback.Stub() {
		...
		public void onOrientationEvent(float yaw, float pitch, float roll) {
			// Log.d(PhotoCompassApplication.LOG_TAG,
			// "FinderActivity: received event from orientation service");
 ...
 */
	private IApplicationModelCallback _applicationModelCallback = new IApplicationModelCallback.Stub() {
		...
		public void onApplicationModelChange() {
			Log.d(PhotoCompassApplication.LOG_TAG,
					"FinderActivity: received event from application model");
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		_photosView = new PhotosView(this, display.getWidth(), display
				.getHeight()
				- statusbarHeight);
		...
		addContentView(_photosView, new LayoutParams(LayoutParams.FILL_PARENT,
				LayoutParams.FILL_PARENT));
		addContentView(controlsView, new LayoutParams(LayoutParams.FILL_PARENT,
				LayoutParams.FILL_PARENT));
	}
	...
	public void onStart() {
		...
		// connect to location service, using hte context of the activity and
		// the class which implements the service
		Intent locationServiceIntent = new Intent(this, LocationService.class);
		// Check if the activity can connect with the service
		_boundToLocationService = bindService(locationServiceIntent,
				_locationServiceConn, Context.BIND_AUTO_CREATE);
		...
		Intent orientationServiceIntent = new Intent(this,
				OrientationService.class);
		_boundToOrientationService = bindService(orientationServiceIntent,
				_orientationServiceConn, Context.BIND_AUTO_CREATE);
		if (!_boundToOrientationService)
			Log.e(PhotoCompassApplication.LOG_TAG,
					"failed to connect to orientation service");
	}
	...
	public void onStop() {
		...
		if (_boundToLocationService) {
			...
			if (_locationService != null) {
				try {
					_locationService
							.unregisterCallback(_locationServiceCallback);
				} catch (DeadObjectException e) {
					// the service has crashed
				} catch (RemoteException e) {
					Log
							.w(PhotoCompassApplication.LOG_TAG,
									"FinderActivity: failed to unregister from location service");
				}
			}
		...
		if (_boundToOrientationService) {
			...
			if (_orientationService != null) {
				try {
					_orientationService
							.unregisterCallback(_orientationServiceCallback);
				} catch (DeadObjectException e) {
					// the service has crashed
				} catch (RemoteException e) {
					Log
	...
	/**
	 * Updates the photo view based on the current location and orientation
	 * parameters.
	 */
	private void _updatePhotoView() {
		...
		List<Photo> photos = Photos.getInstance().getPhotos(_currentAlt,
				_currentLat, _currentLng, _currentYaw,
